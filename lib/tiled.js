...
export function tilesetInfo(level, tilesetName) {
  const ts = level.tilesets?.find(t => t.name === tilesetName) || level.tilesets?.[0];
  if (!ts) throw new Error("No tileset found");
  return {
    ...
    tileproperties: ts.tileproperties || {} // { localId: { collide:true }, ... }
  };
}
...
export function drawLayerData(layer, data, tilesetImg, ts, camX, camY) {
  ...
  const x1 = Math.min(layer.width,  Math.ceil((camX + view.width)  / tw));
  const y1 = Math.min(layer.height, Math.ceil((camY + view.height) / th));
  ...
  for (let ty = y0; ty < y1; ty++) {
    for (let tx = x0; tx < x1; tx++) {
      ...
      const sx = (tid % ts.columns) * tw;
      const sy = Math.floor(tid / ts.columns) * th;

      tilesetImg.startx = sx; tilesetImg.starty = sy;
      tilesetImg.endx   = sx + tw; tilesetImg.endy   = sy + th;
      tilesetImg.width  = tw;      tilesetImg.height = th;

      tilesetImg.draw(Math.fround(tx * tw - camX), Math.fround(ty * th - camY));
    }
  }
}
