...
export function levelEditor_create(tilesetImage, ts, level, fgData, font, player) {
    ...
    const camX = square_x * TILE_SIZE - (448 / 2) + (TILE_SIZE / 2);
    const camY = square_y * TILE_SIZE - (224 / 2) + (TILE_SIZE / 2);
    ...
    font.print(460.0, 195.0, "Mode: " + editMode, Color.new(96, 96, 96));
    ...
    const cursor_x = square_x * TILE_SIZE - camX;
    const cursor_y = square_y * TILE_SIZE - camY;
    Draw.rect(cursor_x, cursor_y, TILE_SIZE, TILE_SIZE, Color.new(255, 0, 0, 128));

    for (const obj of objectLayer.objects) {
        Draw.rect(obj.x - camX, obj.y - camY, obj.width, obj.height, Color.new(0, 0, 255, 128));
    }
